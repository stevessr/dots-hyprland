...
if ! command -v pacman >/dev/null 2>&1; then
  printf "\e[31m[$0]: 未找到 pacman，您的系统似乎不是 ArchLinux 或基于 Arch 的发行版。正在中止...\e[0m\n"
  exit 1
fi
...
startask () {
  ...
  printf '\n'
  printf '本脚本 1. 仅适用于 ArchLinux 和基于 Arch 的发行版。\n'
...
case $SKIP_PLASMAINTG in
  ...
  *)
    if $ask;then
      ...
      echo -e "\e[33m如果您希望 Firefox 中媒体的播放时间显示在音乐控制小部件上，则需要它。\e[0m"
...
case $SKIP_HYPRLAND in
  ...
  *)
    ...
    if [ -d $t ];then
      echo -e "\e[34m[$0]: \"$t\" 已存在，不会执行任何操作。\e[0m"
    else
      echo -e "\e[33m[$0]: \"$t\" 尚不存在。\e[0m"
...
#####################################################################################
printf "\e[36m[$0]: 完成。请查看“手动导入”文件夹并获取您需要的任何内容。\e[0m\n"
printf "\n"
printf "\e[36m建议您查看\n"
...
printf "\e[36m按 \e[30m\e[46m Super+/ \e[0m\e[36m 获取快捷键列表\e[0m\n"
printf "\n"

case $existed_hypr_conf_firstrun in
  y) printf "\n\e[33m[$0]: 警告：\"$XDG_CONFIG_HOME/hypr/hyprland.conf\" 先前已存在。由于这似乎是您的第一次运行，我们已将其替换为新的配置文件。\e[0m\n"
     printf "\e[33m旧文件已被重命名为 \"$XDG_CONFIG_HOME/hypr/hyprland.conf.old\"。\e[0m\n"
;;esac
case $existed_hypr_conf in
  y) printf "\n\e[33m[$0]: 警告：\"$XDG_CONFIG_HOME/hypr/hyprland.conf\" 先前已存在，我们没有覆盖它。\e[0m\n"
     printf "\e[33m请使用 \"$XDG_CONFIG_HOME/hypr/hyprland.conf.new\" 作为正确格式的参考。\e[0m\n"
;;esac
case $existed_hypridle_conf in
  y) printf "\n\e[33m[$0]: 警告：\"$XDG_CONFIG_HOME/hypr/hypridle.conf\" 先前已存在，我们没有覆盖它。\e[0m\n"
     printf "\e[33m请使用 \"$XDG_CONFIG_HOME/hypr/hypridle.conf.new\" 作为正确格式的参考。\e[0m\n"
;;esac
case $existed_hyprlock_conf in
  y) printf "\n\e[33m[$0]: 警告：\"$XDG_CONFIG_HOME/hypr/hyprlock.conf\" 先前已存在，我们没有覆盖它。\e[0m\n"
