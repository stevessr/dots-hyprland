...
handle_file_conflict() {
  ...
  while true; do
    ...
    3)
      ...
      cp -p "$repo_file" "$home_file"
      log_success "已将本地文件备份至 ${filename}.old 并更新为仓库版本"
      break
      ;;
    4)
      cp -p "$repo_file" "${dirname}/${filename}.new"
      log_success "已将仓库版本另存为 ${filename}.new，保留本地文件"
      break
      ;;
    5)
      ...
      b)
        mv "$home_file" "${dirname}/${filename}.old"
        cp -p "$repo_file" "$home_file"
        log_success "已将本地文件备份至 ${filename}.old 并更新"
        break
        ;;
      n)
        cp -p "$repo_file" "${dirname}/${filename}.new"
      ...
      i)
        ...
        log_success "添加 '$relative_path_to_home' 到 $HOME_UPDATE_IGNORE_FILE 并跳过。"
        break
        ;;
      i)
        local relative_path_to_home="${home_file#$HOME/}"
...
build_packages() {
  ...
  "all")
    ...
    ;;
  "select")
    echo -e "\n请输入要构建的软件包名称，以空格分隔（或输入 'all' 选择所有软件包）："
    if ! safe_read "要构建的软件包： " user_selection ""; then
      log_warning "读取输入失败。正在跳过软件包构建。"
      return
    fi
  ...
  if ! safe_read "要继续构建这些软件包吗？ (Y/n): " confirm "Y"; then
    log_warning "读取输入失败。正在跳过软件包构建。"
    return
  fi
  ...
  for pkg_name in "${packages_to_build[@]}"; do
    ...
    if [[ ! -d "$pkg_dir" || ! -f "${pkg_dir}/PKGBUILD" ]]; then
      log_error "找不到软件包或缺少 PKGBUILD: $pkg_name"
      continue
    fi
...
while [[ $# -gt 0 ]]; do
  ...
  -f | --force)
    FORCE_CHECK=true
    log_info "已启用强制检查模式 - 将检查所有文件，忽略 git 变更"
    shift
    ;;
  -p | --packages)
    CHECK_PACKAGES=true
  ...
  -h | --help)
    ...
    echo ""
    echo "软件包模式 (当使用 -p 时):"
    echo "  - 如果没有 PKGBUILD 更改：询问是否仍要检查软件包"
    echo "  - 如果 PKGBUILD 已更改：提供构建已更改的软件包的选项"
    echo "  - 交互式选择要构建的软件包"
    exit 0
    ;;
  --skip-notice)
...
if [[ "$check" == true ]]; then
  ...
  safe_read "继续使用将自担风险 (y/N): " response "N"
...
if ! git diff --quiet || ! git diff --cached --quiet; then
  ...
  if ! safe_read "您想继续吗？这将会储藏您的更改。 (y/N): " response "N"; then
    echo
...
if [[ "$CHECK_PACKAGES" == true ]]; then
  ...
  if [[ ! -d "$ARCH_PACKAGES_DIR" ]]; then
    ...
    if [[ ${#changed_pkgbuilds[@]} -gt 0 ]]; then
      ...
      log_info "自上次更新以来，没有 PKGBUILD 文件发生更改。"
      echo
  ...
  if [[ -d "$ARCH_PACKAGES_DIR" ]]; then
    ...
    if [[ $changed_count -gt 0 ]]; then
      log_warning "注意: 有 $changed_count 个软件包的 PKGBUILD 已更改。请使用 -p 标志来管理软件包。"
    fi
  fi
fi
...
if [[ "$process_files" == true ]]; then
  ...
  for dir_name in "${MONITOR_DIRS[@]}"; do
    ...
    if [[ ! -d "$repo_dir_path" ]]; then
      log_warning "仓库目录未找到: $repo_dir_path"
      continue
    fi
  ...
  log_info "- 已创建新文件: $files_created"
else
  log_info "跳过文件更新 (未检测到更改且未处于强制模式)"
fi
...
if [[ ! -f "$HOME_UPDATE_IGNORE_FILE" && ! -f "$UPDATE_IGNORE_FILE" ]]; then
  ...
  echo "  - 仓库忽略文件: ${REPO_DIR}/.updateignore"
  echo "  - 用户忽略文件: ~/.updateignore"
  echo
  echo "示例模式:"
